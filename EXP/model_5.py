# model with only one resudial at mutli head with rms norm but first mul query-value and then with key
import torch
...
class QVNorm(torch.nn.Module):
    def __init__(self, dim: int):
        super().__init__()
        self.querie_norm = RMSNorm(dim)
        self.value_norm = RMSNorm(dim)
        

    def forward(self, q: Tensor, k: Tensor, v: Tensor) -> tuple[Tensor, Tensor]:
        q = self.querie_norm(q)
        v = self.value_norm(v)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor = None) -> Tensor:
        ...
        energy = torch.einsum('bhqd, bhkd -> bhqk', queries, values)
        print(energy.shape)
        ...
        print(keys.shape, att.shape)
        out = torch.einsum('bhal, bhlv -> bhav', att, keys)
...
class QVNorm_ViT(nn.Sequential):
...
# from torchsummary import summary
# summary(ViT_QVNorm(), (3, 224, 224), device = 'cpu')